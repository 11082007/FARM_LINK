...
import { ShoppingCart, Package, Filter, RefreshCw, AlertCircle, Leaf } from "lucide-react";
...
const BrowseProducts = () => {
  ...
  const hasActiveFilters = searchQuery || filters.category || filters.location || filters.minPrice || filters.maxPrice;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white">Browse Produce</h1>
              <p className="text-gray-600 dark:text-gray-400 mt-1">Fresh from local farms to your table</p>
            </div>
          </div>
          ...
          >
            <ShoppingCart size={24} className="text-gray-700 dark:text-gray-300 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors" />
            {cartItems.length > 0 && (
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-gray-700 dark:text-gray-300 font-medium">
                  Showing <span className="font-bold text-green-600 dark:text-green-400">{filteredListings.length}</span> of {listings.length} products
                </p>
              </div>
              {hasActiveFilters && (
                <button
        ...
        {loading && (
          <div className="flex flex-col justify-center items-center py-20">
            ...
            <p className="mt-6 text-gray-600 dark:text-gray-400 font-medium text-lg">Loading fresh produce...</p>
          </div>
        )}
          ...
          filteredListings.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredListings.map(item => (
                <Card key={item.id || item._id} produce={item} />
              ))}
            </div>
          ) : (
