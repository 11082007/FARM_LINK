...
async function startServer() {
  try {
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(
        `- Server status: http://localhost:${PORT}/api/server-status`
      );
      console.log(`\nAPI Endpoints:`);
      console.log(`- Escrow: http://localhost:${PORT}/api/escrow`);
      console.log(
        `- Check existing users: GET http://localhost:${PORT}/api/existing-users`
      );
      console.log(
        `- Update wallet: POST http://localhost:${PORT}/api/users/:id/wallet`
      );
      console.log(`\nEnvironment: ${process.env.NODE_ENV || "development"}`);
      console.log(`======================================\n`);
      ...
      if (isRailway) {
        ...
        console.log(
          "Run setup: curl -X POST https://farmlink-production.up.railway.app/api/railway-setup"
        );
      } else {
        console.log("RUNNING LOCALLY");
        console.log("IMPORTANT: If login fails, check if Users table has data");
        console.log(
          "Run: curl -X POST http://localhost:3000/api/create-test-users"
        );
        console.log(
          "Reset password: curl -X POST http://localhost:3000/api/reset-test-password"
        );
        console.log("\nTo test connection:");
    ...
    server.on("error", (error) => {
      if (error.code === "EADDRINUSE") {
        ...
        console.log(
          `Or kill the process using: lsof -ti:${PORT} | xargs kill -9`
        );
      } else {
    ...
    if (isRailway) {
      console.error(
        "1. Check if MySQL service is linked to backend on Railway"
      );
      console.error(
        "2. Check Railway Variables for MYSQL* environment variables"
      );
      console.error("3. Ensure NODE_ENV=production is set on Railway");
      console.error(
        "4. Check your config/config.json - it should use process.env for production"
      );
    } else {
